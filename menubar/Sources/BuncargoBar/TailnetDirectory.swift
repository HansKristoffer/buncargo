...
struct RemoteDirectory: Decodable, Sendable {
    ...
    func validate(host: String, expectedID: String? = nil, now: Date = Date()) throws {
        ...
        guard hostname == host, !machineId.isEmpty, expectedID == nil || expectedID == machineId,
            ...
            throw TailnetError("Invalid or stale remote directory")
        }

        for run in runs {
            guard run.id.count <= 256, run.project.count <= 256, run.apps.count <= 100,
...
enum TailnetDirectory {
    ...
    static func fetch(_ peer: TailnetPeer) async throws -> RemoteDirectory {
        ...
        let (bytes, response) = try await session.bytes(from: peer.endpoint)
