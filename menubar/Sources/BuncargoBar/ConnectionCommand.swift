...
enum ConnectionCommand {
    ...
    static func run(_ arguments: [String]) async throws -> Data {
        try await Task.detached(priority: .utility) {
            let candidates = ["bar.json", "connect-coordinator.json"].compactMap {
                name -> RunCLI? in
                let file = RunRegistry.stateDirectory.appendingPathComponent(name)
                return (try? Data(contentsOf: file)).flatMap {
                    try? JSONDecoder().decode(CommandManifest.self, from: $0)
                }?.cli
            }
            let cli = candidates.first { candidate in
                FileManager.default.isExecutableFile(atPath: candidate.program)
                    && (candidate.script.map { FileManager.default.fileExists(atPath: $0) } ?? true)
            }
            let process = Process()
            if let cli {
                process.executableURL = URL(fileURLWithPath: cli.program)
                process.arguments =
                    (cli.script.map { [$0] } ?? []) + ["connect"] + arguments + ["--json"]
            } else {
            ...
            guard process.terminationStatus == 0 else {
                throw ConnectionError(
                    "Connection failed. Run buncargo connect status in a terminal.")
            }
            let size =
                (try FileManager.default.attributesOfItem(atPath: output.path)[.size] as? NSNumber)?
                .intValue ?? 0
            guard size <= 1_048_576 else { throw ConnectionError("Connection response too large") }
