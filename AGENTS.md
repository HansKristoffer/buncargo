...
- `src/core/`
 - `tailnet/` owns Tailscale discovery and sharing. `cli/dev-tailnet.ts` starts one coordinator per machine/sandbox when Tailscale is installed or `TS_AUTHKEY` is set. It reads the existing run registry rather than duplicating lifecycle state. Apps and services with host ports are shared; workers and jobs are skipped. `runtime.ts` adopts an existing login or installs pinned static Linux binaries and starts an isolated userspace node. Never persist enrollment keys or copy authenticated state between sandboxes. The userspace child guard closes the node if its coordinator dies.
 - `tailnet/publisher.ts` gates selected services through loopback TCP on macOS and Linux (Unix backend targets require local-admin permission on Linux and are blocked by the Mac app sandbox); Tailscale Serve owns HTTPS, SSE, WebSocket and TCP transport. `mappings.ts` owns temporary foreground Serve sessions, guarded by the coordinator stdin pipe so abrupt parent death removes its routes. It never resets or removes foreign configuration. The coordinator lifetime lock serializes writers; cleanup disables gates and closes streams, ends Serve sessions, then releases listeners. `discovery.ts` probes only Tailscale peers and validates directory identity and target hostnames. Menu actions open validated URLs directly and reuse the local target row components. See `docs/tailscale.md`.
