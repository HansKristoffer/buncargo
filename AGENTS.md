...
- `src/core/`
 - `connect/` owns recipient identity, directory access and authenticated HTTP/TCP forwarding. `cli/dev-connect.ts` activates only for nonempty `BUNCARGO_CONNECT_TOKENS` and shares every selected app/service with a host port. Workers, jobs and portless containers are skipped; custom services default to TCP. `connect-directory/` is the discovery-only Worker; keep it out of npm bundles. `connect/tailcat/` installs the pinned binary and owns recipient-isolated Tailcat processes. Tailcat addresses contain a pre-shared key and may only be read with device owner credentials. Each forwarding process uses a fresh client key to avoid DERP identity collisions. Publisher loopback gates restrict forwarding to the selected targets and close active streams when a target stops. Renewal failure closes the recipient's publisher; never replay interrupted streams. The menu invokes the CLI and never executes remote metadata. No Worker traffic relay, JWT capabilities, or full Tailscale setup.
 - Connection transport ownership is centralized: `connect/transport/sockets.ts` handles socket tracking and half-close bridges for publishers and local forwards; `browser-access.ts` owns browser authorization. `connect/forwards.ts` owns listeners and sibling browser peers per session, while `helper.ts` serializes control actions and polling. Keep port/readiness validation and bounded request/response JSON reading in `connect/protocol.ts`. Tailcat delegates download, checksum, archive extraction and cache receipts to `core/tool-install.ts`; its own binary module only selects the pinned asset and supplies registry headers. CI, releases and local verification use `bun run test:integration-tailcat`. See `docs/cloud-connect-plan.md` for module boundaries and lifecycle invariants.
