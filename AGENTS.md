...
- `src/core/`
 ...
 - `tailnet/publisher.ts` gates selected services through Unix sockets; Tailscale Serve owns HTTPS, SSE, WebSocket and TCP transport. `mappings.ts` journals Serve ownership before mutations and never resets or removes foreign configuration. The coordinator lifetime lock serializes writers; cleanup closes streams before removing mappings. `discovery.ts` probes only Tailscale peers and validates directory identity and target hostnames. Menu actions open validated URLs directly and reuse the local target row components. See `docs/tailscale.md`.
