...
- `src/docker-compose/`
 ...
 - `serviceFingerprint` and `buncargo.service-hash` govern reuse per service, independent of selected subsets. Include effective interpolation, user labels and referenced definitions; exclude only self-referential hash metadata. Unverifiable external inputs must reconcile. `projectStackHash` remains a compatibility export, not the warm-start decision.
...
- `src/container-runtime/`
 ...
 - `ensureServicesRunning` skips `up` only when each selected service is running with a matching service fingerprint and every relevant input can be proven unchanged. Old unlabeled containers reconcile once. Both runtime adapters offer async cancellable operations; sync exports remain compatibility wrappers.
...
- `src/core/`
 - `connect/` owns remote sharing over iroh. There is no server of ours: two computers dial each other by ed25519 key, over QUIC, encrypted end to end, falling back to a public relay when no direct path can be punched. `cli/dev-connect.ts` registers per-run recipient intent only when `BUNCARGO_CONNECT_TOKENS` is configured; the run registry remains the lifecycle authority. Tokens never reach child app environments.
 - `connect/identity.ts` mints both keys with `node:crypto`, because an iroh endpoint ID *is* an ed25519 public key. That is what keeps the 13 MB native addon out of `connect token` and out of CLI startup; only the coordinator loads it, through `connect/iroh.ts`. `connect/bundle.ts` copies the addon beside the coordinator bundle under the same hash, since a `.node` file cannot be bundled and the bar runs that bundle from outside any `node_modules`.
 - `connect/publisher.ts` holds one connection per recipient and multiplexes every run shared with it; QUIC's per-stream flow control is why a stalled database client cannot block a page load. It resolves each opened stream against the run registry and process birth identity, so a target whose process was replaced is refused rather than served. `connect/receiver.ts` is the other half: it authenticates a publisher's secret, then binds a loopback listener per target, because a browser cannot dial an iroh endpoint. The connection *is* the lease — there are no leases, renewals or server clocks to keep. Revocation closes the connection and refuses that publisher key from then on.
 - `connect/stream.ts` is the framing: newline-delimited JSON for the handshake, then raw bytes, which is why the reader hands back what it read past the newline. Shared local/remote menu rows stay in Swift's existing target components. See `docs/remote.md`.
 ...
 - `secrets/infisical.ts` fetches an app's Infisical scope once per dev run and `process/dev-servers.ts` puts the values in the child's environment, below `process.env` and below the computed env. The injection sits in `startDevServers` because that is the one function both spawn paths reach: `buncargo dev` spawns the apps itself and never calls `startAppServers`, so 9.2.0 shipped the whole feature as dead code for the CLI. For the same reason the config-level defaults are resolved into `app.secrets` in `environment/context.ts` rather than passed alongside — a spawner cannot forget a field that is already on the app it was handed. It exists because the app-side loaders shell out to the Infisical CLI at startup, and concurrent Infisical CLI processes hang indefinitely — three apps in one monorepo is enough. The cache is keyed on the whole scope so two apps in one project share a fetch, and the CLI runs under a `withFileLock` on `~/.buncargo/infisical-cli` so parallel worktrees queue instead of racing. A failure warns once and starts the app anyway: the app's own loader is still there. Never log a value, and never put the CLI's stderr in an error — it can carry secret material.
 ...
 - `file-lock.ts` guards every shared read-modify-write using kernel flock on a persistent `.lock.v2` inode. Never unlink that inode or steal a live holder. Death releases the kernel lock; acquisition times out with an error and never executes unlocked. Bun FFI loads lazily on macOS/Linux; Windows users need WSL. Stop old dev sessions and restart the hosts service when upgrading protocols.
...
- Start heartbeat ownership before preparation. Reconciliation and delayed watchdog teardown share a project gate and recheck live owners. One run exiting must never condemn another run's services.
