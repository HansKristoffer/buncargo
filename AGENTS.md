...
- `src/core/`
 ...
 - `secrets/infisical.ts` fetches an app's Infisical scope once per dev run and `environment/servers.ts` puts the values in the child's environment, below `process.env` and below the computed env. It exists because the app-side loaders shell out to the Infisical CLI at startup, and concurrent Infisical CLI processes hang indefinitely — three apps in one monorepo is enough. The cache is keyed on the whole scope so two apps in one project share a fetch, and the CLI runs under a `withFileLock` on `~/.buncargo/infisical-cli` so parallel worktrees queue instead of racing. A failure warns once and starts the app anyway: the app's own loader is still there. Never log a value, and never put the CLI's stderr in an error — it can carry secret material.
