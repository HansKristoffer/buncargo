...
- `src/core/`
 - `connect/` owns recipient identity, directory access and authenticated HTTP/TCP forwarding. `cli/dev-connect.ts` activates only for nonempty `BUNCARGO_CONNECT_TOKENS` in long-running dev runs and selected `expose: true` targets. `connect-directory/` is the separately deployed Worker and recipient relay; keep it out of npm bundles. Private traffic uses outbound WebSockets to the stable directory origin, never Quick Tunnels. `relay.ts` bounds in-flight bytes using per-direction credit, and checks live grants on admission/renewal; `transport/publisher.ts` only dials configured loopback targets. The Worker must set `binaryType = "arraybuffer"` before accepting sockets. Publish transport readiness separately from app readiness, and never replay interrupted streams. Copied tokens authorize registration only; private device credentials authorize reading and connecting. The menu bar invokes the CLI for setup and connections, and never executes paths received from remote metadata.
