...
- `src/container-runtime/`
 ...
 - `sweep.ts` is the one cleanup policy. `decideSweep` is pure: a stack nobody owns comes down when its checkout is gone, when every container is stopped, when its run released it longer ago than its idle hold, or when its owner died past the grace; a running stack with no entry at all is left alone. `sweepOrphanedContainers` applies it to every stack on the machine, each under its project's lifecycle lock (`project-lock.ts`, a busy lock is skipped, not failed), and then retires entries whose containers are gone — but only when a runtime answered, since an empty listing from a dead daemon means "cannot tell". The watchdog (`core/watchdog-runner.ts`, one per machine, started by `ensureWatchdog`) is this on a timer and exits when nothing is left to watch; `ls` and `doctor` run it inline, so a killed watchdog is never the last line of defence.
 - There is **one** liveness record: `~/.buncargo/runs.json`. `environment/run-claim.ts` publishes a run's entry *before* its first container exists, so the sweep can never see a fresh stack as unowned, and the entry carries the idle hold, the runtime and the pinned binary — which is why the sweep needs no config. The library claims too, not only the CLI: a `dev.start()` from a script owns containers the same way and used to be invisible here. Releasing sets `releasedAt` and keeps the entry (a run with no services is withdrawn instead); `isRunAlive` is what every other reader filters on, so a released entry is visible only to the sweep. `core/watchdog.ts` is now just `ensureWatchdog` and its pid file. Two locks, and it needs both: the runner holds `.runner` for its whole life, which is the "is one running?" answer a `kill -9` cannot fake, and a caller holds `.spawn` across spawn-and-confirm so two `dev` runs starting together cannot each launch one. Collapsing them looks tempting and deadlocks — the spawner would be holding the lock its own child must take.
