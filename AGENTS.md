...
- `src/docker-compose/`
 ...
 - `projectStackHash` hashes the selected services *after* interpolation, so a port block that moved changes it even though the file text did not. The generated model carries `buncargo.stack-hash: ${BUNCARGO_STACK_HASH:-}` — a reference, not a value, because the hash depends on ports that are allocated after the file is written. Both backends substitute it through the interpolation they already do. Labels are excluded from the hash, since the hash is one of them.
...
- `src/container-runtime/`
 ...
 - `ensureServicesRunning` skips `up()` only when every selected service is running from exactly this stack, which `projectStackHash` decides. `up` used to run unconditionally because it is the only place either backend compares a running container against the config it was started from — skipping it made an edited image, port or env var take effect only after a manual `--down`. The hash *is* that comparison, made explicit: it covers the interpolated definition of every selected service, so anything that would change a container changes it, and only an exact match skips. Worth it because `docker compose up` on an unchanged stack still costs most of a second on a command that runs constantly. A container with no hash — created before the label existed — reads as "cannot compare" and reconciles.
...
- `src/core/`
 ...
 - `run-registry.ts` is `~/.buncargo/runs.json`: what is running on this machine. It exists because nothing on disk answered that — the heartbeat, the route registry, the tunnel registry and container labels each knew part of it, and `ls` needed Docker to say a project was up. It mirrors the hosts route registry on purpose (same list-registry primitive, same lock, same prune-by-pid), and `claimRun` answers a second run in the same root with `keep` for the same reason `classifyRouteClaim` does: that run is reusing the first one's servers, so the live run stays the owner and the entry disappears when the process owning those servers exits. Written `0600` — it carries the dev database password.
 ...
 - `timing.ts` is the `--timing` / `BUNCARGO_TIMING` phase breakdown. `createNoopPhaseTimer` exists so `run-cli.ts` never branches on whether timing is on.
