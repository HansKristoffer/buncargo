...
import { makeSecret, parseConnectionToken, relayEndpoint } from "./protocol";
import { startRelayPublisher } from "./transport/publisher";

test("CLI creates a private identity and its detached helper opens and disconnects a remote app", async () => {
	...
	let connector: ReturnType<typeof startRelayPublisher> | undefined;
	...
	try {
		...
		connector = startRelayPublisher({
			...
			targets: [{ id: "web", port: app.port as number, status: "ready" }],
