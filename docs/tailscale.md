...
Browser targets open `https://machine.tailnet.ts.net:port/` directly. Ports are allocated from the worktree path and target name, so restarting a worktree keeps its address while the machine identity and available ports remain the same. Database clients connect to the same machine and the displayed TCP port. Database rows offer TablePlus; the deeplink carries the dev credentials, retargeted at the tailnet address, and is only served to authenticated tailnet peers. The menu reuses the local environment and target row components, grouped by project with branch/worktree and machine names.
...
- `publisher.ts` derives targets once per refresh and uses a byte-stream gate for each target: loopback TCP on macOS and Linux. Unix backend targets require local-admin permission on Linux and are rejected by the Mac app's sandboxed network extension. The userspace daemon still uses a private Unix control socket. It checks run/app process birth identity when accepting streams. Retirement disables the gate and closes its streams, ends the Serve session, then releases the listener. Closing a gate is idempotent.
