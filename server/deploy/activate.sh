...
[[ "$release" =~ ^/opt/buncargo-connect/releases/[a-f0-9]{40}$ ]] || {
...
[[ -s /etc/buncargo-connect/cloudflare.token ]] || {
  echo 'Save the zone DNS token in /etc/buncargo-connect/cloudflare.token first' >&2
  exit 1
}
if [[ ! -f /etc/buncargo-connect/server.env ]]; then
  (
    umask 077
    printf 'CONNECT_STORAGE_KEY=%s\n' "$(openssl rand -hex 32)" >/etc/buncargo-connect/server.env
  )
fi
...
for component in directory caddy frps; do
  case "$component" in
    directory)
      ...
      environment='/etc/buncargo-connect/server.env'
      extra=''
      ;;
    caddy)
      executable='caddy run --config /opt/buncargo-connect/current/Caddyfile --adapter caddyfile'
      environment='/etc/buncargo-connect/caddy.env'
      extra='AmbientCapabilities=CAP_NET_BIND_SERVICE'
      ;;
    frps)
      executable='frps -c /opt/buncargo-connect/current/frps.json'
      environment='/etc/buncargo-connect/server.env'
      extra=''
      ;;
  esac
...
for attempt in $(seq 1 60); do
  if bash "$release/cert-sync.sh"; then
    break
  fi
  sleep 2
done
